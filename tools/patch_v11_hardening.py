...
def replace_between(text,start,end,new):
    a=text.find(start)
    if a<0: raise RuntimeError(f'missing start anchor: {start}')
    b=text.find(end,a+len(start))
    if b<0: raise RuntimeError(f'missing end anchor: {end}')
...
if needle not in s: raise RuntimeError('constants anchor missing')
s=s.replace(needle,needle+"const MiB=1024*1024,DIRECT_LIMIT=8*MiB,CHUNK_SIZE=4*MiB,PREVIEW_LIMIT=64*MiB;",1)
...
s=replace_between(s,'function dispose(){','function fit',new_dispose)
...
"""
s=replace_between(s,'async function loadItem(i){','async function exportGLB',new_load)
...
"""
s=replace_between(s,'async function prepare(){','const b64=',new_prepare)
...
"""
if 'async function upload(){' not in s: raise RuntimeError('upload anchor missing')
s=s.replace('async function upload(){',helpers+'async function upload(){',1)
...
"""
# remove the helper copy plus original upload body by replacing from the first upload declaration to the UI bindings
start=s.find(helpers+'async function upload(){')
if start<0: raise RuntimeError('inserted upload helper anchor missing')
end=s.find("$('#filesBtn')",start)
if end<0: raise RuntimeError('UI binding anchor missing')
s=s[:start]+helpers+new_upload+s[end:]
...
if r.exists():
    ...
    if '## V11 hardened upload pipeline' not in text:
